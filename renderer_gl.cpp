...
/*!
\brief Creates the game window and a rendering context
\param w width of the window in pixels
\param h height of the window in pixels
\param title title of the window
\warning There can only be one (highlander) window at a time.
*/
void RendererGL::create_window(int w, int h, const char* title)
{
    assert(w>0 && h>0);
    assert(title != nullptr);
    assert(window_ == nullptr);
    if(window_) {
        printf("Error: Requested to create a new window when one already exists.\n");
        assert(false);
    }
    else {
        window_ = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
        if(!window_) {
    ...
    glOrtho(0.0, w, 0.0, h, -10.0, 100.0);
...
//    std::vector<uint32_t> fontSizes = {16, 32};
//    load_font("DroidSansMono.ttf", fontSizes);
}

/*!
\brief destroy the current window
*/
void RendererGL::destroy_window()
{
    assert(window_!=nullptr);
    SDL_DestroyWindow(window_);
    window_ = nullptr;
